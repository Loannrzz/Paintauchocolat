...
then
    ...
    if [ $GHCR_REPOSITORY_STATUS -eq 0 ]; then
        echo "Je suis un coding style et j'aime pas le c donc je vous fait chier avce mes regles a la con..."
        $BASE_EXEC_CMD pull ghcr.io/epitech/coding-style-checker:latest && $BASE_EXEC_CMD image prune -f
        echo "C'est bon mon chakal a la crème fouété au fouet ! <3"
    else
